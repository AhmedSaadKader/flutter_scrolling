...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    for (int i = 0; i < itemsCount; ++i) {
      final int randomColorNumber = (math.Random().nextDouble() * 0xFFFFFF).toInt();
      ...
      return Scaffold(
        body: FlexibleScrollbar(
          ...
          scrollLabelBuilder: (info) {
            ...
            final double cellSize = (isVertical ? screenSize.width : screenSize.height) / 3;
            final int lineNum = (scrollController.position.pixels ~/ cellSize) + 1;
            return AnimatedContainer(
              ...
              return Container(
                ...
                child: Center(
                  child: Text(
                    (++index).toString(),
                    style: TextStyle(
                      color: randomColor.computeLuminance() > 0.5 ? Colors.black : Colors.white,
                    ),
                  ),
                ),
              );
